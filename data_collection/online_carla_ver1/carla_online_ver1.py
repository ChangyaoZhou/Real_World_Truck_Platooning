    ...
    ##################### data collection ################################
        for frame in range(800):
            ...
            if frame > 39 and frame % 10 == 0:
                ...
                ## write down raw states and control values for off-trajectory data     
                np.set_printoptions(threshold = np.inf)
                f1 = open("./data_collection/online_carla_ver1/output/model_input.py","a")
                f1.write(str(repr(np.array(state_collect)))) 
                f2 = open("./data_collection/online_carla_ver1/output/model_gt.py","a")
                ...
                state_collect_i.extend(new_ref_state)
                state_collect_i.extend(new_ego_state) 
                state_collect_on.append(state_collect_i)
                u_collect_on.append([throttle, str_angle])
        ...
        ## write down raw states and control values for on-trajectory data
        np.set_printoptions(threshold = np.inf)
        f1 = open("./data_collection/online_carla_ver1/output/model_input.py","a")
        f1.write(str(repr(np.array(state_collect_on)))) 
        f2 = open("./data_collection/online_carla_ver1/output/model_gt.py","a")
        f2.write(str(repr(np.array(u_collect_on)))) 
