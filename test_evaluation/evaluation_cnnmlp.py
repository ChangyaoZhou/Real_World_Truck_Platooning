...
# import for model
from model.my_models import MyModel_MLP_transform, MyModel_CNN1
...
FIG_SIZE = [8,8]
INPUT_SIZE = [400,400]
MODEL_PATH_CNN = './model/trained_models/mynet_CNN_twosteps.pth'
...
def Control_Disfference(control_target, control_ego, row_ind, col_ind):
    control_difference = np.array([L2_Distance(control_target[r], control_ego[c]) for r,c in zip (row_ind, col_ind)])
...
def main():
    ...
    try: 
        ...
        color1 = bp1.get_attribute('color').recommended_values[1]
        ...
        for frame in range(1000):
            ...
            global Not_Complete
            if (absolute_distance>=20 or (v_t_fol <0.3 and v_t_lead > 1.0))and Not_Complete != 3:                
                Not_Complete += 1
