...
def main():
    ...
    try:
        ...
        camera_bp_rgb.set_attribute('image_size_x',  str(IMG_SIZE[2]))
    ...
    ##################### data collection ################################
        for frame in range(799):
            ...
            if frame > 29 and frame % 10 == 0:
                ...
                f1 = open("./output/town04/model_input_off_4.py","a")
                f1.write(str(repr(np.array(state_collect)))) 
                f2 = open("./output/town04/model_gt_off_4.py","a")
                f2.write(str(repr(np.array(u_collect)))) 
                vehicle_lead.set_transform(transform_lead)                
                vehicle_lead.set_target_velocity(v_lead_vec)
        ...
        '''        
        np.set_printoptions(threshold = np.inf)
        f1 = open("./output/town04/model_input_off.py","a")
        f1.write(str(repr(np.array(state_collect))))
        f2 = open("./output/town04/model_gt_off.py","a")
        f2.write(str(repr(np.array(u_collect))))'''
