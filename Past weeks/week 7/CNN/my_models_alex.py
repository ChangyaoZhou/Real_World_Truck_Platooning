...
class MyModel_CNN(nn.Module): 
    ...
    def forward(self, x):
        img, ref_v, ego_v = x
        img = img.view(-1, 3, 150, 200).to(torch.float32)
        features = self.features(img)  #output: N*256*3*5
        ref_v = ref_v.view(-1, 1, 1, 1).repeat(1, 1, 3,5).to(torch.float32) #shape: N*1*3*5
        ego_v = ego_v.view(-1, 1, 1, 1).repeat(1, 1, 3,5).to(torch.float32) #shape: N*1*3*5
