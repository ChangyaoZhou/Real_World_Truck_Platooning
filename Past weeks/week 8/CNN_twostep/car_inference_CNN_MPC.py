...
# import for model
from model.my_models import MyModel_CNN_2step, MyModel1
...
FIG_SIZE = [8,8]
INPUT_SIZE = [400,400]
MODEL_PATH_CNN = './model/mynet_cnn_img1.pth'
MODEL_PATH_MLP = './model/mynet_relative_pose.pth' 



def run_mpc(mpc_controller, ref_state, ego_state, u_frame, bounds):
    ...
    u_solution = minimize(mpc_controller.cost_function, u_frame, (ego_state, ref_state), 
...
def visualize_image(image1, image2, plt):
    ...
    rgb2_3channels = data_reshaped[:,:,:3] # first 3 channels 
...
def main():
    ...
    try: 
        ...
        for frame in range(800):
            ...
            output_CNN = mynet_CNN(model_input)# (deltax, deltay,deltayaw)
            input_MLP = torch.cat((output_CNN[0], torch.tensor([v_t_lead,v_t_fol])), axis = 0) 
            # model inference for MLP  
            print('gt input',-gt_input)
            print('CNN output',input_MLP)
            state_ref = [0, 0, 0, v_t_lead]
            output_CNN = output_CNN.detach().numpy()
            state_fol = [output_CNN[0][0], output_CNN[0][1], output_CNN[0][2], v_t_fol]
            print(output_CNN[0][0])
            
            throttle, str_angle = run_mpc(mpc, state_ref, state_fol, u, input_bounds) 
