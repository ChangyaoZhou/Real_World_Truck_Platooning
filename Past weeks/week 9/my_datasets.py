...
class MyDataset2(torch.utils.data.Dataset):  
    def __init__(self,root, txtname, transform=None, size=None, target_transform=None): 
        ...
        input_list = [] 
        label_list = np.empty([0, 5], dtype=float)       
        for line in fh:                #按行循环txt文本中的内容
            ...
            label_list = np.append(label_list, np.array([[float(words[3]), float(words[4]),\
                                                          float(words[5]), float(words[6]),float(words[7])]]), axis=0)
    ...
    def __getitem__(self, index):   
        ...
        return inputs, self.label_list[index][0], self.label_list[index][1], \
                       self.label_list[index][2], self.label_list[index][3], self.label_list[index][4]
...
class MyDataset4(torch.utils.data.Dataset):  
    def __init__(self,root, txtname, transform=None, size=None, target_transform=None): 
        self.root = root
        self.txtname = txtname
        fh = open(root + self.txtname, 'r') #按照传入的路径和txt文本参数，打开这个文本，并读取内容
