...
def main():
    ...
    try: 
        ...
        color3 = bp3.get_attribute('color').recommended_values[1]
        ...
        print('created the second ego vehicle %s' % vehicle_follow_follow.type_id)
        ...
        for frame in range(800):
            ...
            #### the second ego vehicle
            v_fol__fol_vec = np.array([vehicle_follow_follow.get_velocity().x, vehicle_follow_follow.get_velocity().y,
                                  vehicle_follow_follow.get_velocity().z])
            v_t_fol_fol = np.linalg.norm(v_fol__fol_vec)
            ...
            throttle = float(throttle) / 10.0
            str_angle = float(str_angle)/ 10.0
            control_values.append([throttle, str_angle])   
            u_i = np.append(u_i, np.array([(throttle, str_angle)]), axis=0)  
            print('first throttle', throttle)
            print('first str_angle', str_angle)
            ...
            model_input = np.concatenate((ref_sec_i[-1][None,:], state_sec_i[-1][None,:]), axis = 1)
            output_pred_sec = mynet(torch.tensor(model_input).to(torch.float32)) 
            throttle_sec, str_angle_sec = output_pred_sec[0].detach().numpy()
            throttle_sec = float(throttle_sec) / 10.0
            str_angle_sec = float(str_angle_sec)/ 10.0
            control_values_sec.append([throttle_sec, str_angle_sec])   
            u_sec_i = np.append(u_sec_i, np.array([(throttle_sec, str_angle_sec)]), axis=0)  
            print('second throttle', throttle_sec)
            print('second str_angle', str_angle_sec)
