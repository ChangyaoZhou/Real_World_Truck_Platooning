...
from control_lead_list import control_value_lead
...
def main():
    ...
    try:
        ...
        #vehicle_lead.apply_control(carla.VehicleControl(throttle=control_value_lead[0][0], steer=control_value_lead[0][1]))
        ...
        camera_bp.set_attribute('image_size_y',  str(300))
        camera_bp.set_attribute('fov',  str(130.0))
        camera_transform = carla.Transform(carla.Location(x=1.8, z=1.3))
        camera = world.spawn_actor(camera_bp, camera_transform, attach_to=vehicle_follow)
        actor_list.append(camera)
        print('created %s' % camera.type_id)
        camera.listen(lambda image: image.save_to_disk('output/stereo/TrueDepth/%02d_%04d.png' % (FLAG, frame)))
        '''
        ...
        camera_rgb2.listen(lambda image: image.save_to_disk('output/stereo/RGB1/%02d_%04d.png' % (FLAG, frame)))
        ...
        camera_transform_rgb3 = carla.Transform(carla.Location(x=1.8,y=0.5,z=1.3))
        ...
        camera_rgb3.listen(lambda image: image.save_to_disk('output/stereo/RGB2/%02d_%04d.png' % (FLAG, frame)))
    ...
    ##################### data collection ################################
        ...
        for frame in range(1000): 
            ...
            world.tick()
            #vehicle_lead.apply_control(carla.VehicleControl(throttle=control_value_lead[frame+1][0], steer=control_value_lead[frame+1][1]))
            vehicle_lead.set_autopilot(True)
            ...
            if frame > 309 and frame % 10 == 0:
                ...
                for n in range(N_sample):
                    ...
                    for f in range(N_frame):   
                        #world.apply_settings(settings)
                        world.tick() 
                        vehicle_lead.apply_control(carla.VehicleControl(throttle=control_value_lead[frame+f+2][0], steer=control_value_lead[frame+f+2][1])) 
                        ...
                        f_data = open('./output/town04_image_data/offdata_image.txt','a+')
                        print('images/%06d_%02d_%02d.png' % (frame,n+1,f+1),ref_i[-1][0], state_i[-1][0], ref_i[-1][1], state_i[-1][1], ref_i[-1][2], state_i[-1][2], ref_i[-1][3], state_i[-1][3], throttle, str_angle, file = f_data)
                        f_data.close()
            ...
            f_data = open('./output/stereo/data.txt','a+')
