...
FIG_SIZE = [8,8]
INPUT_SIZE = [800,300]
MODEL_PATH_CNN = './models/mynet_stereo_2.pth'
...
def Lane_Invation_handler(event):
    ## print the collision
    print('Lane Invation!')

    #change the infraction number
    global Infraction
    Infraction *= 0.8
...
def Collision_handler(event):
    ...
    global collision_ids
    #print('collision_ids', collision_ids)
    if other_actor_id not in collision_ids:        
        collision_ids.append(other_actor_id)
        print('Collision with ', other_actor)
...
def main():
    ...
    try: 
        ...
        world = client.load_world("/Game/Carla/Maps/Town%02d"%TOWN_NUM) 
        ...
        transform1 = world.get_map().get_spawn_points()[SPAWN_POINT]
        ...
        blueprint_library = world.get_blueprint_library()
        Lane_invasion_sensor = world.spawn_actor(blueprint_library.find('sensor.other.lane_invasion'),
                                                carla.Transform(), attach_to=vehicle_follow)
        Lane_invasion_sensor.listen(lambda event: Lane_Invation_handler(event))
        ...
        for frame in range(1000):
            ...
            if absolute_distance>=20 or (v_t_fol <0.3 and v_t_lead > 1.0):
                global Not_Complete
                Not_Complete += 1
        ...
        print('Town%02d SpawnPoint%03d Inftaction:%f Conpletion:%f'%(TOWN_NUM, SPAWN_POINT, Infraction, Current_frame/1000), file = f_data)
        f_data.close()
        print('done.')
