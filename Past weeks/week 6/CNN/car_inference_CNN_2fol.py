...
FIG_SIZE = [8,8]
INPUT_SIZE = [400,300]
MODEL_PATH = './models/mynet_alexnet_all_5.pth'
...
def main():
    ...
    try: 
        ...
        color1 = bp1.get_attribute('color').recommended_values[0]  #red
        color2 = bp2.get_attribute('color').recommended_values[0]  #red
        color3 = bp2.get_attribute('color').recommended_values[1]  #black
        ...
        camera_transform_rgb = carla.Transform(carla.Location(x=1.8, z=1.3))
        ...
        camera_bp_rgb3.set_attribute('image_size_x',  str(INPUT_SIZE[0]))
        camera_bp_rgb3.set_attribute('image_size_y',  str(INPUT_SIZE[1]))
        camera_bp_rgb3.set_attribute('fov',  str(100))
        camera_transform_rgb3 = carla.Transform(carla.Location(x=0.0, z=2.0))
        ...
        for frame in range(800):
            ...
            img_input = img_reshaped[:,:,:3].reshape((3, rgb2_list[-1].height, rgb2_list[-1].width))
            #img_reshaped = np.reshape(img, (4, rgb2_list[-1].height, rgb2_list[-1].width))
            #img_input = img_reshaped[:3,:,:] # first 3 channels
            print('image shape', img_input.shape)
            model_input = [torch.tensor(img_input), torch.tensor(v_t_lead), torch.tensor(v_t_fol)] 
            
            #### for the second ego vehicle 
            img2 = np.array(rgb3_list[-1].raw_data)
            img2_reshaped = np.reshape(img2, (rgb3_list[-1].height, rgb3_list[-1].width, 4))
            img2_input = img2_reshaped[:,:,:3].reshape((3, rgb3_list[-1].height, rgb3_list[-1].width))
            #img_reshaped = np.reshape(img, (4, rgb2_list[-1].height, rgb2_list[-1].width))
            #img_input = img_reshaped[:3,:,:] # first 3 channels
            #print('image shape', img_input.shape)
            ...
            #### for the second ego vehicle
            output_pred2 = mynet(model_input2) 
            #print(output_pred)
            throttle2, str_angle2 = output_pred2[0].detach().numpy()
            throttle2 = float(throttle2)  
            str_angle2 = float(str_angle2) 
            control_values2.append([throttle2, str_angle2])   
            u_sec_i = np.append(u_sec_i, np.array([(throttle2, str_angle2)]), axis=0)  
            print('throttle2', throttle2)
            print('str_angle2', str_angle2)
